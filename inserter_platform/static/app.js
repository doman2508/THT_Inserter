...
function operatorMarkerSizeForZoom(zoomPercent) {
  ...
  return Math.round(Math.max(15, Math.min(28, 17 * markerScale)));
}
...
function showActionPanel(panelName) {
  ...
  if (["editProject", "boardImage", "reimportProject", "addLine"].includes(panelName) && !state.selectedProject) {
...
function fillEditProjectForm(project) {
  ...
  els.editProjectForm.elements.status.value = project.status || "draft";
}
...
function isOperatorProject(project) {
  return ["ready", "active"].includes(project?.status);
}
  ...
  return `
    <section class="readiness-panel ${readiness.ready ? "ready" : "blocked"}">
      <div class="readiness-head">
        ...
        <button type="button" class="primary" data-release-project ${readiness.ready && !published ? "" : "disabled"}>${escapeHtml(actionLabel)}</button>
      </div>
...
function boardBaseMetrics(project, points) {
  ...
  return looksSwapped
    ? { width: projectHeight, height: projectWidth, swapped: true }
    : { width: projectWidth, height: projectHeight, swapped: false };
}
...
function renderBoardMap(project, points, options = {}) {
  ...
  const operatorMarkerSize = markerClass === "operator-board-marker"
    ? operatorMarkerSizeForZoom(Number(options.zoomPercent) || 100)
    : null;
  ...
  return `
    <div class="operator-commandbar">
      ...
      <div class="operator-view-controls">
        ...
        <button type="button" data-operator-center>Centruj</button>
        <button type="button" class="${view.focusMode ? "active" : ""}" data-operator-focus>Focus</button>
        <button type="button" class="${view.showAllPoints ? "active" : ""}" data-operator-toggle-points>Reszta: ${view.showAllPoints ? "ON" : "OFF"}</button>
...
function operatorActiveMarkers() {
  return Array.from(els.operatorSteps.querySelectorAll('[data-board-marker="active"]'));
}
...
function setOperatorBoardZoom(zoom, baseZoom = null) {
  ...
  if (stage) {
    stage.style.setProperty("--operator-marker-size", `${operatorMarkerSizeForZoom(safeZoom)}px`);
  }
...
function attachOperatorViewControls() {
  ...
  root.querySelector("[data-operator-toggle-points]")?.addEventListener("click", () => {
    updateOperatorView({ showAllPoints: !state.operatorView.showAllPoints });
  });
  ...
  root.querySelector("[data-operator-center]")?.addEventListener("click", centerOperatorBoardOnCurrent);
